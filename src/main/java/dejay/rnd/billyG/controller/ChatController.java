        ...
        if (StringUtils.isEmpty(contentDto.getMessage()) && contentDto.getTransactionIdx() != 0 && contentDto.isSystemYn() != true) {
            ...
            if (contentDto.getStep()== 0 ) {
                ...
                CompletableFuture.runAsync(() -> {
                    try {
                        //렌탈 오너가 취소를 눌렀을 때
                        pushService.sendPush(new Long[]{findTr.getUser().getUserIdx()}, findTr.getRental().getUser().getUserIdx(), findRoom.getChatRoomIdx(), findTr.getRental().getRentalIdx(),
                ...
                CompletableFuture.runAsync(() -> {
                    try {
                        //렌탈러가 취소를 눌렀을 때
                        pushService.sendPush(new Long[]{findTr.getRental().getUser().getUserIdx()}, findTr.getUser().getUserIdx(), findRoom.getChatRoomIdx(), findTr.getRental().getRentalIdx(),
                ...
                findRental.setUpdateAt(FrontUtil.getNowDate());
                ...
                if (findTr.getOwnerStatus() == 70 ) {
                    CompletableFuture.runAsync(() -> {
                        try {
                            //렌탈완료 알림 - 렌탈오너에게
                            pushService.sendPush(new Long[]{findTr.getRental().getUser().getUserIdx()}, findTr.getUser().getUserIdx(), findRoom.getChatRoomIdx(),findTr.getUser().getUserIdx(),
...
//                    rentalService.updateRental(findRental);
                    ...
                    CompletableFuture.runAsync(() -> {
                        try {
                            ...
                            pushService.sendPush(new Long[]{transaction.getUser().getUserIdx()}, transaction.getRental().getUser().getUserIdx(), findRoom.getChatRoomIdx(), transaction.getRental().getRentalIdx(),
